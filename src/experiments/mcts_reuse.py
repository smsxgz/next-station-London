"""Run resumable fixed-configuration MCTS subtree-reuse benchmarks."""
...
def policy_name(
    ...
    *,
    ...
    name = f"mcts-uct-{simulations}-c{exploration_label}-reuse"
...
def run_mcts_reuse_game(
    ...
    policy = ReuseMCTSPolicy(simulations, exploration=exploration)
    ...
    max_tree_depth = tree_nodes_max = 0
    reuse_attempts = reuse_hits = 0
    retained_nodes = pruned_nodes = retained_root_visits = 0
    search_seconds = prune_seconds = 0.0
    ...
    while game.status == "playing":
        ...
        tree_nodes_max = max(tree_nodes_max, stats.tree_nodes)
        reuse_attempts += int(stats.reuse_attempted)
        reuse_hits += int(stats.reuse_hit)
        retained_nodes += stats.retained_nodes
        pruned_nodes += stats.pruned_nodes
        retained_root_visits += stats.retained_root_visits
    ...
    record = build_game_record(
        ...
        algorithm={
            ...
            "decision_nodes_mean": decision_nodes / decisions,
            "tree_nodes_max": tree_nodes_max,
            "reuse_attempts": reuse_attempts,
            "reuse_hits": reuse_hits,
            "reuse_hit_rate": (
                reuse_hits / reuse_attempts if reuse_attempts else 0.0
            ),
            "retained_nodes_total": retained_nodes,
            "retained_nodes_mean_per_hit": (
                retained_nodes / reuse_hits if reuse_hits else 0.0
            ),
            "pruned_nodes_total": pruned_nodes,
            "retained_root_visits_total": retained_root_visits,
            "retained_root_visits_mean_per_hit": (
                retained_root_visits / reuse_hits if reuse_hits else 0.0
            ),
            ...
            "subtree_prune_seconds": prune_seconds,
        },
    )
    ...
    *,
    ...
    for seed in seeds:
        ...
        if (
            ...
            or algorithm.get("rollout_policy") != "greedy"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate chance-sampled UCT with retained subtrees"
    )
    ...
    def accept(result: CompletedGame) -> None:
        ...
        print(
            f"game {len(records)}/{game_count}: seed={result.seed}, "
    ...
    if args.workers == 1:
        for task in tasks:
            accept(run_mcts_reuse_game(task))
    else:
        with ProcessPoolExecutor(max_workers=args.workers) as executor:
            futures = [executor.submit(run_mcts_reuse_game, task) for task in tasks]
            for future in as_completed(futures):
                accept(future.result())

    selected = [records[seed] for seed in seeds]
    algorithms = [record["algorithm"] for record in selected]
    reuse_attempts = sum(item["reuse_attempts"] for item in algorithms)
    reuse_hits = sum(item["reuse_hits"] for item in algorithms)
    retained_nodes = sum(item["retained_nodes_total"] for item in algorithms)
    retained_visits = sum(
        item["retained_root_visits_total"] for item in algorithms
    )
    ...
    if reuse_attempts:
        print(
            ...
            f"{retained_visits / reuse_hits if reuse_hits else 0.0:.1f}",
            flush=True,
        )
    score = summary["score"]
    print(
